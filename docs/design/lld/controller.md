...
| The same over **six hours** | **6.792 s** on an unbroken connection; 22.83 s spanning a drop; 13 connections | `measured` | run `20260909T130306Z`, 21,610 s, 2026-09-09. `reconnect_after` = 45 s is 6.6x above the worst live-socket gap, which **bounds** the value without picking it: the tag stays `assumed`. [../quiet-gap.md](../quiet-gap.md) |
| `degraded_after` | 15 s | `assumed` | Three ticker refreshes at `measured` 5001 ms (`feed.py`) picked it. The hour crossed it twice, both times on an interruption rather than a quiet market — which is what the badge is for; run `20260909T130306Z` never crossed it on an unbroken connection at all. Both are evidence bounding the value, not its origin |
| `reconnect_after` | 45 s | `assumed` | Three degraded intervals, under Delta's documented 60 s idle disconnect so we notice before the venue drops us, and above Delta's own 35 s heartbeat window. **The untagged hour's worst gap missed it by 0.215 s.** Run `20260909T130306Z` bounds it at 6.6x the worst gap on an unbroken connection — the same role the row two above gives it, and the role [../cloud/controller-policies.md](../cloud/controller-policies.md) C3 and [../decisions/0003-controller-policy.md](../decisions/0003-controller-policy.md) give it. #59 confirmed the value unchanged on documents. The tag is `assumed` here, in C3, in 0003 and in [../../../CONTEXT.md](../../../CONTEXT.md) §5 |
