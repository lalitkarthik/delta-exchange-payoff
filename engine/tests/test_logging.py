...
def test_no_colour_is_attached_when_standard_error_is_not_a_terminal(
    ...
    appear anywhere. Structural, not a per-line check: the coloured handler is never
    attached at all when `is_terminal` says no."""
    ...
    assert isinstance(logger.handlers[0], DailyFileHandler)
