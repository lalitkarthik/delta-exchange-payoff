 ...
 * fixed-column — nine fields a side, written once and reversed — and nothing else here
 * holds a table whose columns are data. So this borrows its
 * idioms rather than their code: the ladder's at-the-money marking, its hatch for a cell
 * that is not there, and `lib/format.ts` for every number. The stylesheet block is
 * self-contained under `.structures` rather than leaning on a shared table recipe, so it
 * cannot drift when one arrives.
 *
...
function StructureCell({
  ...
  return (
    <td
      className={`num ${value > 0 ? "up" : value < 0 ? "down" : ""}`}
