 ...
 *
 * `ExposureScreen` is the precedent and this follows it line for line where it can — the
 * two pickers, `loadExpiries`, one `subscribeChain`, the status chip, the `replaceState`
 * URL sync. What it does not share is a shell: the exposure screens draw charts off one
 * projection with no parameters, and this one is a grid whose shape the reader chooses.
 * Six controls is not a prop, it is a screen.
 *
 * **Live only, and there is no scrubber**, for `ExposureScreen`'s reason: the board is
 * read for where the market stands now, and the stored day would need a read path that
 * does not exist. So the URL carries the underlying and the expiry and no minute — a
 * minute in this address would promise a fixed board the screen does not have.
 *
    ...
    <main className="screen">
      ...
      <h2 className="exposure-heading">
        ...
        {coverage && coverage.total > 0 ? (
          <span className="exposure-coverage">
