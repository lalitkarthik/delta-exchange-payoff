 ...
 * components, so anything computed inside one ships unverified; `lib/exposure.ts` set
 * that precedent and this file keeps it. Everything below can be asserted from
 * `bun tests/structures.test.ts` against a literal chain.
 *
 ...
 * of the underlying and `contract_value` is never applied here, which is the rule the
 * whole stack states: the lot is carried so a screen can multiply, not so a solver can.
 *
 ...
 * draw and no honest way to pick one, so nothing is drawn — the same rule `gexByStrike`
 * follows for a missing multiplier.
 */
